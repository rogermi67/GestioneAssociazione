...
import { FiPlus, FiCheck, FiTrash2, FiUser, FiClock } from 'react-icons/fi'
...
export default function TodoList({ eventoId }) {
  ...
  const handleCreate = async (e) => {
    e.preventDefault()
    try {
      await todoEventiAPI.create({ ...formData, eventoId })
  ...
  const handleAssegna = async (todoId, socioId, collaboratoreId) => {
    try {
      await todoEventiAPI.assegna({ todoEventoId: todoId, socioId, collaboratoreId })
        ...
        {todos.map(todo => (
          <div key={todo.todoEventoId} className={`p-4 border rounded ${todo.stato === 'Completato' ? 'bg-gray-50' : 'bg-white'}`}>
            <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {todo.assegnazioni.map(ass => (
                          <span key={ass.assegnazioneId} className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">
                            {ass.socio ? ass.socio.nomeCompleto : ass.collaboratore ? `${ass.collaboratore.nome} ${ass.collaboratore.cognome}` : ''}
                          </span>
                        ))}
                      </div>
                    </div>
              ...
              <button onClick={() => handleDelete(todo.todoEventoId)} className="text-red-600 hover:text-red-800">
                <FiTrash2 size={18} />
              </button>
            </div>
          </div>
